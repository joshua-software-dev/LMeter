...
public class MeterWindow : IConfigurable
{
    ...
    [JsonIgnore]
    private List<Combatant> _lastSortedCombatants = new ();
    [JsonIgnore] 
    ...
    {
        var newMeter = new MeterWindow(name);
    ...
    {
        if (!this.GeneralConfig.Preview && !this.VisibilityConfig.IsVisible()) return;

        var localPos = pos + this.GeneralConfig.Position;
        var size = this.GeneralConfig.Size;
        ...
        var combat = CharacterState.IsInCombat();
        if (this.GeneralConfig.ReturnToCurrent && !_lastFrameWasCombat && combat) _eventIndex = -1;

        this.UpdateDragData(localPos, size, this.GeneralConfig.Lock);
        var needsInput = !this.GeneralConfig.ClickThrough;
        ...
        (
            ...
            drawList =>
            {
                if (_unlocked && _lastFrameWasDragging)
                {
                ...
                if (this.GeneralConfig.ShowBorder)
                {
                    ...
                    var borderSize = size;
                    if (this.GeneralConfig.BorderAroundBars && this.HeaderConfig.ShowHeader)
                    {
                        borderPos = borderPos.AddY(this.HeaderConfig.HeaderHeight);
                        borderSize = borderSize.AddY(-this.HeaderConfig.HeaderHeight);
                    }

                    for (var i = 0; i < this.GeneralConfig.BorderThickness; i++)
                    {
                ...
                if (this.GeneralConfig.Preview && !_lastFrameWasPreview) _previewEvent = ActEvent.GetTestData();

                var actEvent = this.GeneralConfig.Preview
                    ? _previewEvent
    ...
    {      
        if (actEvent?.Combatants is null || !actEvent.Combatants.Any()) return;

        var sortedCombatants = this.GetSortedCombatants(actEvent, this.GeneralConfig.DataType);
        var top = this.GeneralConfig.DataType switch
        {
        ...
        var maxIndex = Math.Min(i + this.BarConfig.BarCount, sortedCombatants.Count);
        for (; i < maxIndex; i++)
        {
            var combatant = sortedCombatants[i];
            combatant.Rank = (i + 1).ToString();

            var current = this.GeneralConfig.DataType switch
            {
            ...
            var barColor = this.BarConfig.BarColor;
            var jobColor = this.BarColorsConfig.GetColor(combatant.Job);
    ...
    private bool DrawContextMenu(string popupId, out int selectedIndex)
    {
        ...
        if (!ImGui.IsAnyItemActive() && !ImGui.IsMouseClicked(ImGuiMouseButton.Left)) ImGui.SetKeyboardFocusHere(0);

        if (ImGui.Selectable("Current Data")) selected = true;

        var events = PluginManager.Instance.ActClient.Current.PastEvents;
        if (events.Count > 0) ImGui.Separator();
    ...
    private List<Combatant> GetSortedCombatants(ActEvent actEvent, MeterDataType dataType)
    {
        ...
        var sortedCombatants = actEvent.Combatants.Values.ToList();
        sortedCombatants.Sort
        (
            (x, y) =>
            {
                var xFloat = dataType switch
                {
